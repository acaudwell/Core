...
void UILayout::updatePos(const vec2f& pos) {

    switch(alignment) {
        case UI_LAYOUT_ALIGN_TOP_LEFT:
            this->pos = vec2f(0.0f, 0.0f);
            break;
        case UI_LAYOUT_ALIGN_BOTTOM_LEFT:
            this->pos = vec2f(0.0f, display.height - rect.y);
            break;
        case UI_LAYOUT_ALIGN_TOP_RIGHT:
            this->pos = vec2f(display.width-rect.x, 0.0f);
            break;
        case UI_LAYOUT_ALIGN_BOTTOM_RIGHT:
            this->pos = vec2f(display.width-rect.x, display.height - rect.y);
            break;
        default:
            this->pos = pos;
            break;
    ...
    if(right_align) {
        cursor = this->pos + vec2f(rect.x - margin.x, margin.y);
    } else {
        cursor = this->pos + margin;
    }

    foreach(UIElement* e, elements) {

        if(right_align) {
            e->updatePos(cursor - vec2f(e->rect.x, 0.0f));
        } else {
            e->updatePos(cursor);
        }
...
UIElement* UILayout::elementAt(const vec2f& pos) {
